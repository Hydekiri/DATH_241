...
const setupDateFilter = () => {
    ...
    filterButton.addEventListener('click', () => {
        ...
        if (filteredHistory.length === 0) {
            const historyContainer = document.querySelector(".printers-rows");
            historyContainer.innerHTML = '<div class="no-data">Không có dữ liệu trong khoảng thời gian này</div>';
        } else {
            renderHistory();
        }
    });
...
const setupSearch = () => {
    ...
    searchInput.addEventListener("input", (event) => {
        ...
        filteredHistory = allHistory.filter(record => 
            ...
            (record.printer?.branchName || '').toLowerCase().includes(searchTerm) ||
            (record.paperSize || '').toLowerCase().includes(searchTerm) ||
            new Date(record.printStart).toLocaleDateString('vi-VN').includes(searchTerm)
        );
